...
public class slimeType : EnemyBase
{
    ...
    protected override IEnumerator ChasePlayer()
    {
        ...
        while (count < 1000)
        {
            if (
                separateCount > 0
                && gameObject.GetComponent<Health>().getCurrentHP()
                    <= (gameObject.GetComponent<Health>().getHP()) * 0.5f
            )
            {
                Debug.Log("体力半分");
    ...
    {
        GameObject slime1 = Instantiate(
            Resources.Load<GameObject>("Objects/Enemy/slime"),
            gameObject.transform.position - new Vector3(plusMinus, 2, 0),
            Quaternion.identity
        );
